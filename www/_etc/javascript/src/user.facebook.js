NB.User.facebook = {
  clicked: function(){
    $('.fb-button').hasClass('fb-login')?NB.User.facebook.login():NB.User.facebook.logout();
  },
  check: function(){
  ...
  loggedin: function(auth){
    ...
    FB.api("/me", function(response,auth) {
      ...
      NB.Ajax.html(
        ...
        NB.User._container,
        NB.User._script_fb,
        'fbuid='+response.id+'&fb_first_name='+response.first_name+'&fb_last_name='+response.last_name+'&fb_link='+encodeURIComponent(response.link)+'&sent_signup_url='+encodeURIComponent(NB.crumb.canonical)+'&sig='+sig+'&authstring='+encodeURIComponent(authstring),
        false
      );
    });
  },
