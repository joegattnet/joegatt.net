...
#    	push(@chartvalues, $target_counter{$character});
  ...
  $location = untaint("../../_cache/enface--anagram_table-b=$b.json");
  open ANAGRAM_TABLE,">$location";
  print ANAGRAM_TABLE "[$values_string]";
  close ANAGRAM_TABLE;
  
  $location = untaint("../../_cache/enface--anagram_table-b=$b.html");
  open ANAGRAM_TABLE,">$location";
  ...
  $location = untaint("../../_cache/enface--version_index-b=$b.html");
...
sub cache_refresh {
  ...
  foreach $file (@files) {
    $cachefile = "../../_cache/$file";
    $cachefile = untaint($cachefile);
    if($file =~ /archive=true/){
      #Add a datestamp to filename
      $archivefile = "../../_cache_archive/$file";
      use File::Copy;
      move($cachefile,$archivefile);
    } else {
      unlink($cachefile);
    }
    #Priming cache
    $url = "http://$serverName/_cache/$file";
    #$url =~ s/\&scope=[a-z]//;
