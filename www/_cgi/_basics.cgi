...
# ******************************************************************************
    ...
    $twitter_pwfileloc = '/home/admin/_db_passwords/joegatt-net/twitter.txt';
		$localhost = false;
  	if ($serverName eq 'dev.joegatt.org') {
    	$debug = true;
    	$dsn = 'DBI:mysql:joegatt-net-dev:localhost';
    }	elsif ($serverName eq 'test.joegatt.org') {
    	$debug = true;
...
$date_format = qq~
  CASE 
    WHEN DATEDIFF(UTC_TIMESTAMP(),comments.date_created)=0 THEN 
      CASE
        WHEN TIMESTAMPDIFF(MINUTE, comments.date_created,UTC_TIMESTAMP())<1 THEN 'just now'
...
sub saveAnagramParagraph {
  ...
  my $target_text = $_[2];
    ...
    $sql = "SELECT text FROM target_text WHERE sequence=? AND book_id=? ORDER BY date_created DESC LIMIT 1;";
    my $sth = $dbh->prepare("$sql");
    $sth->execute($sequence,$b);
    $sth->finish();
...
#    	push(@chartvalues, $target_counter{$character});
  ...
  open ANAGRAM_TABLE,">../../_cache/wutz/_anagram_table.json";
  print ANAGRAM_TABLE "[$values_string]";
  close ANAGRAM_TABLE;
  
  open ANAGRAM_TABLE,">../../_cache/wutz/_anagram_table.html";
  print ANAGRAM_TABLE $anagram_table;
  close ANAGRAM_TABLE;
  
  $version_number = eval ("1 - 0.$anagram_total");
  open VERSION_NUMBER,">../../_cache/wutz/_version_jpsw.html";
