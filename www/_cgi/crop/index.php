...
include 'header.php' ?>
  ...
  if($_GET["show"]=='raw'){
    if ($handle = opendir('../../_resources/raw')) {
        ...
        if($file != "." && $file != "..") {
          ...
          print "<li style=\"float:left;display:inline-block;margin:0 5px 5px 0;\">
            <a href=\"crop_image2.php?image=$image&ext=$ext&aspectWidth=$aspectWidth&aspectHeight=$aspectHeight\">
              <img src=\"../../_resources/cut/$image-0-$aspectRatio-th-120-0-0-0.$ext\" height=\"120\" title=\"$image $aspectWidth:$aspectHeight\"/>
            </a></li>";
        }
      }
      closedir($handle);
    ...
    if ($handle = opendir('../../_resources/templates')) {
      while (($file = readdir($handle)) !== false) {
